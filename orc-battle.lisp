...
;; Random value that is always at least 1
(defun randval (n)
  (1+ (random (max 1 n))))
...
(defmethod monster-hit (m x)
  (decf (monster-health m) x) ;; variant of setf to substract
  (if (monster-dead m)
      (progn (princ "You killed the ")
             (princ (type-of m)) ;; type-of: find the type of any Lisp value
             (princ "! "))
...
;; include: inheritance?
;; Dynamic typing + functions as first-class citizens
;; No need to add inheritance related code (?)
(defstruct (orc (:include monster)) (club-level (randval 8)))
(push #'make-orc *monster-builders*)

;; Specializing monster-show
(defmethod monster-show ((m orc))
  (princ "A wicked orc with a level ")
  (princ (orc-club-level m))
  (princ " club"))

;; Specializing monster-attack
